...
<template>
    ...
    <div>
        ...
        <PaperCard
            ...
            abstract="Many economic studies have identified barriers to migration which can distort labor allocation and productivity, but comparing the macroeconomic importance of each barrier is complicated by variation in study design and sampling. We build and estimate a dynamic general-equilibrium model of rural--urban migration which nests several of the key frictions emphasized in the literature. We discipline the model using large-scale data from Kenya---the site of several recent migration experiments---and reproduce each experiment within the model. We proceed to estimate the long-run treatment effects of each intervention, the macroeconomic effects of scaling interventions to the national level, and the aggregate importance of each migration friction. Our findings shed light on the lack of a migration response to large unconditional cash transfers, and reveal how treatment impacts on migration can rise over time as the constraints relaxed by an intervention interact with subsequent productivity shocks."
        />
        ...
        <PaperCard
            ...
            status="under review, "
            date="updated September 2026"
            abstract="Information interventions are often evaluated by whether they affect adoption, even though their economic value depends on the gains that adoption generates. We study information dissemination in the context of rural-urban migration, where spatial income gaps are large and the returns to moving are heterogeneous. In a large-scale, cluster-randomized experiment in rural Kenya, we test three delivery modes: providing information about the capital city privately to rural households, facilitating private conversations between rural households and urban workers, and broadcasting information publicly in villages. Each mode increases migration by similar amounts. Yet both private modes generate significant economic gains, while public dissemination does not. We find that private delivery influences those with less migration experience---who exhibit higher income gains at the margin---compared to public dissemination. We build and estimate a model of constrained migration to rationalize these patterns and quantify the role of selection on experience in explaining the economic returns across delivery modes. Our results show that the value of information depends not only on whether it changes behavior but also on which households respond."
            registry_name="AEA RCT Registry"
            registry_link="https://www.socialscienceregistry.org/trials/10051"
            policy_name="VoxDev"
            policy_link="https://voxdev.org/topic/migration-urbanisation/what-causes-people-migrate-cities"
        />
